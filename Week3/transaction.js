...
async function seedData(){
  ...
  try{
    await Promise.all(execQuery(commit),execQuery(transaction),execQuery(updateAccount1),execQuery(rollBack),
   execQuery(updateAccount2),execQuery(insertData),execQuery(commitEnd) )
  } catch(error){
    console.error(error)
  }
  connection.end()
